...
namespace SpynTest
{
    ...
    {
        [Test]
        public void WriteReadDescription() => Assert.That(new SyncPair
        {
            Description = "test"
        }.Description, Is.EqualTo("test"));

        [Test]
        public void WriteReadIsFullSync() => Assert.That(new SyncPair
        {
            IsFullSync = true
        }.IsFullSync, Is.EqualTo(true));

        [Test]
        public void WriteReadSyncType() => Assert.That(new SyncPair
        {
            SyncType = "Full"
        }.SyncType, Is.EqualTo("Full"));

        [Test]
        public void WriteReadSourceRoot() => Assert.That(new SyncPair
        {
            SrcRoot = "c:\\src"
        }.SrcRoot, Is.EqualTo("c:\\src"));

        [Test]
        public void WriteReadDestinationRoot() => Assert.That(new SyncPair
        {
            DstRoot = "c:\\dst"
